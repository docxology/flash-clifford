...
@torch.compile
def gelu_sgp_norm_2d_torch(x, y, weight_expanded, normalize=True):
    """Geometric product layer with GELU activation and RMS normalization in 2D Clifford algebra."""
    x = mv_gelu(x)
    y = mv_gelu(y)
    o = torch.einsum("ibn, nijk, kbn -> jbn", x, weight_expanded, y)
    if normalize:
        o = mv_rmsnorm_2d(o)
    return o
...
@torch.compile
def gelu_sgp_norm_3d_torch(x, y, weight_expanded, normalize=True):
...
@torch.compile
def gelu_fcgp_norm_2d_torch(x, y, weight_expanded, normalize=True):
    """Fully connected geometric product layer with GELU activation and RMS normalization in 2D Clifford algebra."""
    x = mv_gelu(x)
    y = mv_gelu(y)
    o = torch.einsum("ibn, mnijk, kbn -> jbm", x, weight_expanded, y)
    if normalize:
        o = mv_rmsnorm_2d(o)
    return o
...
@torch.compile
def gelu_fcgp_norm_3d_torch(x, y, weight_expanded, normalize=True):
